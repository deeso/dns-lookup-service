...
LOG_CONF_FILE=$CONFIGS_DIR/log_config.yaml
...
cp $LOG_CONF_FILE log_config.yaml
# hack
...
echo "./$RUST_BIN_NAME --config config.toml --log_config log_config.yaml --iron_server" > rust_cmd.sh
cat rust_cmd.sh
...
# clean up here
rm -fr config.toml log_config.yaml rust_cmd.sh $RUST_BIN_NAME $TMP_DIR
